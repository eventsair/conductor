...
## 1.0.1 GENERAL GUIDELINES FOR INTERACTIVE QUESTIONS
**PROTOCOL: These guidelines apply to ALL interactive questioning sections in this document. You MUST follow them whenever you are asking the user questions.**
...
*   **3. Interaction Flow:**
    ...
    *   Confirm your understanding by summarizing before moving on.
    ...
    - **Structure:**
        ...
        E) [Autogenerate and review <document_name>]

*   **4. Source of Truth for Generation:** The source of truth for generation is **only the user's selected answer(s)**. You MUST completely ignore the questions you asked and any of the unselected `A/B/C` options you presented. Take the user's chosen answer and synthesize it into a well-formed section for the document. You are encouraged to expand on the user's choice to create a comprehensive and polished output. DO NOT include the conversational options (A, B, C, D, E) in the final file.

*   **5. Auto-Generate Logic:** If the user selects the autogenerate option, immediately stop asking questions for that section. Use your best judgment to infer the remaining details based on previous answers and project context, generate the full document content, write it to the file, and proceed to the next section.
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            ...
            -   **2.1 File Size and Relevance Triage:**
                1.  **Respect Ignore Files:** Before scanning any files, you MUST check for the existence of `__IGNORE_FILE__` and `.gitignore` files. If either or both exist, you MUST use their combined patterns to exclude files and directories from your analysis. The patterns in `__IGNORE_FILE__` should take precedence over `.gitignore` if there are conflicts. This is the primary mechanism for avoiding token-heavy, irrelevant files like `node_modules`.
                ...
                3.  **Fallback to Manual Ignores:** ONLY if neither `__IGNORE_FILE__` nor `.gitignore` exist, you should fall back to manually ignoring common directories. Example command: `ls -lR -I 'node_modules' -I '.m2' -I 'build' -I 'dist' -I 'bin' -I 'target' -I '.git' -I '.idea' -I '.vscode'`.
...
2.  **Ask Questions Sequentially:** Ask one question at a time, following the **General Guidelines for Interactive Questions** defined in Section 1.0.1. Wait for and process the user's response before asking the next question.
...
3.  **Draft the Document:** Once the dialogue is complete (or the autogenerate option is selected), generate the content for `product.md`. You are encouraged to expand on the gathered details to create a comprehensive document.
4.  **User Confirmation Loop:** Present the drafted content to the user for review and begin the confirmation loop.
    ...
    >
    > You can always edit the generated file with __EDITOR_HINT__ after this step.
    > Please respond with A or B."
...
3.  **Draft the Document:** Once the dialogue is complete (or the autogenerate option is selected), generate the content for `product-guidelines.md`. You are encouraged to expand on the gathered details to create a comprehensive document.
4.  **User Confirmation Loop:** Present the drafted content to the user for review and begin the confirmation loop.
...
3.  **Draft the Document:** Once the dialogue is complete (or the autogenerate option is selected), generate the content for `tech-stack.md`. You are encouraged to expand on the gathered details to create a comprehensive document.
4.  **User Confirmation Loop:** Present the drafted content to the user for review and begin the confirmation loop.
...
2.  **Select Code Style Guides:**
    -   List the available style guides by running `ls __TEMPLATE_PATH__/code_styleguides/`.
    ...
    -   **Action:** Construct and execute a command to create the directory and copy all selected files. For example: `mkdir -p conductor/code_styleguides && cp __TEMPLATE_PATH__/code_styleguides/python.md __TEMPLATE_PATH__/code_styleguides/javascript.md conductor/code_styleguides/`
...
1.  **Copy Initial Workflow:**
    -   Copy `__TEMPLATE_PATH__/workflow.md` to `conductor/workflow.md`.
...
3.  **Ask Questions Sequentially:** Ask one question at a time, following the **General Guidelines for Interactive Questions** defined in Section 1.0.1. Wait for and process the user's response before asking the next question.
